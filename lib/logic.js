...
async function inspectMaterial(tx) {
    ...
    const supplierRegistry = await getParticipantRegistry('org.acme.construction.Manufacturer');

    // update participant
    await supplierRegistry.update(tx.material.supplier);
...
async function inspectWork(tx) {
    ...
    // get participant registry
    const builderRegistry = await getParticipantRegistry('org.acme.construction.SubContractor');

    // update participant
    await builderRegistry.update(tx.work.builder);
...
async function processRCO(tx) {
    ...
    const oco = factory.newResource('org.acme.construction', 'OwnerChangeOrder', 'oco-' + tx.rco.getIdentifier());
    ...
    oco.scopeOfWork = [];
    oco.createDateTime = oco.approveDateTime;
    oco.submitDateTime = oco.approveDateTime;
    oco.description = 'OCO - ' + tx.rco.getIdentifier();
